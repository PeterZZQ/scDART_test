...
result_dir = "./results_hyperparams/"
    ...
    for data_name in ["bifur1", "bifur2", "bifur3","bifur4", "bifur5", "bifur6","bifur7", "bifur8", "bifur9"]:
        for ts in [[10, 30, 50], [20, 40, 60], [30, 50, 70]]:
            ...
            *_, ave_score = calc_score(z_rna, z_atac, pt_infer, cell_labels_rna, cell_labels_atac, data_name)
            ...
            scores = scores.append({"data_name": data_name, "ts": ts, "num_anchors": num_anchors, "kendall-tau": bmk.kendalltau(pt_infer, pt_true), "ave_score": ave_score}, ignore_index=True)

            # save files
            np.save(file = result_dir + data_name + "_" + str(ts) + "_" + str(num_anchors) + "_z_rna.npy", arr = z_rna)
            np.save(file = result_dir + data_name + "_" + str(ts) + "_" + str(num_anchors) + "_z_atac.npy", arr = z_atac)

            utils.plot_latent(z1 = z_rna_pca, z2 = z_atac_pca, anno1 = rna_dataset.cell_labels, 
                                anno2 = atac_dataset.cell_labels, mode = "separate",
                                save = result_dir + data_name + "_" + str(ts) + "_" + str(num_anchors) + "_clust.png", 
            ...
            utils.plot_latent(z1 = z_rna_pca, z2 = z_atac_pca, anno1 = rna_dataset.is_anchor.numpy(), 
                                ...
                                save = result_dir + data_name + "_" + str(ts) + "_" + str(num_anchors) + "_anchor.png", 
                                figsize = (20,7), axis_label = "PCA")

            utils.plot_latent(z1 = z_rna_pca, z2 = z_atac_pca, anno1 = rna_dataset.cell_labels, 
                                anno2 = atac_dataset.cell_labels, mode = "modality",
                                save = result_dir + data_name + "_" + str(ts)+ "_" + str(num_anchors) + "_mod.png", 
...
sns.boxplot(data = scores, x = "ts", y = "kendall-tau", hue = "num_anchors", ax = ax)
ax.set_ylim([0,1])
fig.savefig(result_dir + "kt.png", bbox_inches = "tight")

fig = plt.figure(figsize = (10,7))
ax = fig.add_subplot()
scores = pd.read_csv(result_dir + "score.csv", index_col = 0)
sns.boxplot(data = scores, x = "ts", y = "ave_score", hue = "num_anchors", ax = ax)
ax.set_ylim([0,0.5])
ax.set_ylabel("Lineage separation score")
fig.savefig(result_dir + "ave_score.png", bbox_inches = "tight")
